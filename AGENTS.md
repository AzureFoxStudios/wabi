...
4. **`packages/wabi-protocol` is generated.** `cargo test -p wabi-core --features ts` REGENERATES the `.ts` files and STRIPS manual edits — you must re-append `"category"|"lore"` to `ChannelType.ts` and `position`/`parentId` to `ChannelView.ts` after any regen.
