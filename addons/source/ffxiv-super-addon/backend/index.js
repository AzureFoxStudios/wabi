...
function sanitizeString(input, maxLen = 240) {
    if (typeof input !== 'string')
        return '';
    return input.trim().slice(0, maxLen);
}
function sanitizeChannelId(input) {
...
async function persist() {
    if (!ctxRef)
        return;
    await ctxRef.storage.set(STORAGE_KEY, [...channels.values()]);
}
async function loadState(ctx) {
    ...
    for (const candidate of stored) {
        if (!candidate || typeof candidate !== 'object')
            continue;
        const channelId = sanitizeChannelId(candidate.channelId);
        if (!channelId)
            continue;
        channels.set(channelId, {
            channelId,
            pinnedCards: Array.isArray(candidate.pinnedCards) ? candidate.pinnedCards.slice(0, MAX_PINS) : [],
            raidNotes: Array.isArray(candidate.raidNotes) ? candidate.raidNotes.slice(0, MAX_NOTES) : [],
            wipeLogs: Array.isArray(candidate.wipeLogs) ? candidate.wipeLogs.slice(0, MAX_WIPES) : [],
            templates: Array.isArray(candidate.templates) ? candidate.templates.slice(0, MAX_TEMPLATES) : [],
...
async function fetchXivApiResource(resource, id) {
...
async function searchXivApi(index, query) {
...
function makeFallbackCard(kind, title, detail, pinKey) {
    return {
...
function pickSummary(value, keys) {
...
function toLookupCard(kind, value, detailFallback, link) {
    const title = pickSummary(value, ['Name', 'name', 'Title', 'title']) ||
        pickSummary(value, ['Singular', 'Plural']) ||
        detailFallback;
    const subtitle = pickSummary(value, ['Description', 'description', 'Text', 'text']) ||
...
async function lookupItem(query) {
    ...
    if (!cleaned)
        return [makeFallbackCard('item', 'Item lookup unavailable', 'Provide an item name or item ID.')];
    const id = Number(cleaned);
    if (Number.isFinite(id) && id > 0) {
        const payload = await fetchXivApiResource('item', id);
        if (payload)
            return [toLookupCard('item', payload, cleaned, `${XIVAPI_BASE}/item/${id}`)];
    }
...
async function lookupGeneric(kind, index, resource, query) {
    const cleaned = sanitizeString(query);
    if (!cleaned)
        return [makeFallbackCard(kind, `${kind} lookup unavailable`, 'Provide a search term.')];
    const id = Number(cleaned);
    if (Number.isFinite(id) && id > 0) {
        const payload = await fetchXivApiResource(resource, id);
        if (payload)
            return [toLookupCard(kind, payload, cleaned, `${XIVAPI_BASE}/${resource}/${id}`)];
    }
...
async function lookupMarket(itemQuery, world, dataCenter) {
    ...
    return [
        makeFallbackCard('market', best?.title || `Item ${itemId}`, `Universalis did not return a market snapshot for ${locationHint}.`)
    ];
}
async function lookupProfile(request) {
    ...
    return [
        makeFallbackCard('profile', 'Runtime profile unavailable', 'XIVAPI can back static cards, but live character / free-company runtime state is not guaranteed here.')
    ];
}
function emitState(ctx, channelId) {
    ctx.emitToChannel(channelId, 'ffxiv:state', getChannelState(channelId));
}
const plugin = {
    ...
    routes: [
        {
            ...
            handler: async (req, res) => {
                ...
                res.json({ success: true, state: getChannelState(channelId) });
            }
        },
        {
            method: 'post',
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (!channelId || !card?.id) {
                    res.status(400).json({ success: false, error: 'channelId and card are required' });
                    return;
                }
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (!channelId || !cardId) {
                    res.status(400).json({ success: false, error: 'channelId and cardId are required' });
                    return;
                }
                const state = getChannelState(channelId);
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                touchState(state);
                await persist();
                res.json({ success: true, state, note });
            }
        },
        {
            method: 'post',
            ...
            handler: async (req, res) => {
                ...
                if (!channelId || !encounter || !reason) {
                    res.status(400).json({ success: false, error: 'channelId, encounter, and reason are required' });
                    return;
                }
                const state = getChannelState(channelId);
                const wipe = {
                ...
                touchState(state);
                await persist();
                res.json({ success: true, state, wipe });
            }
        },
        {
            method: 'post',
            ...
            handler: async (req, res) => {
                ...
                touchState(state);
                await persist();
                res.json({ success: true, state, template });
            }
        }
    ],
    socketHandlers: {
        'ffxiv:get-state': (socket, data) => {
            const channelId = sanitizeChannelId(data?.channelId);
            if (!channelId) {
                socket.emit('ffxiv:error', { message: 'channelId is required' });
                return;
            }
            socket.emit('ffxiv:state', getChannelState(channelId));
        }
    },
