		...
		if (req.url === '/v1/intents/create') {
			...
			res.end(
				JSON.stringify({
					...
					presentation: {
						mode: 'payment_link',
						url: `https://mock-west-payments.local/checkout/${encodeURIComponent(providerIntentId)}`,
						expiresAt
					},
					expiresAt,
					metadata: {
		...
		if (req.url === '/v1/intents/refund') {
			...
			res.end(
				JSON.stringify({
					providerIntentId,
					status: 'refunded',
					providerRefundId: `mock_west_ref_${providerIntentId.slice(-8)}`,
					metadata: { adapter: 'mock-west', refunded: true }
				})
			);
			return;
		}
...
try {
	...
	process.env.WEST_PAYMENTS_ADAPTER_BASE_URL = adapterBaseUrl;
	process.env.WEST_PAYMENTS_ADAPTER_TOKEN = 'sandbox-west-token';
	process.env.WEST_PAYMENTS_WEBHOOK_SECRET = 'sandbox-west-webhook-secret';

	const pluginModulePath = path.resolve(__dirname, '..', '..', 'plugins', 'western-payments', 'backend', 'index.mjs');
	...
	assert(capabilities.methods.some((method: any) => method.id === 'card_checkout'), 'card_checkout method missing');

	const createInput = {
		intentId: 'wabi_west_intent_sandbox_001',
		...
		description: 'Western sandbox test',
		customerRef: 'acct_west_demo_001',
		idempotencyKey: 'west-sandbox-intent-idem-001',
		metadata: { flow: 'west-sandbox' }
	};
	...
	const refund = await plugin.payment.refundIntent(ctx, {
		intentId: createInput.intentId,
		providerIntentId: created.providerIntentId,
		idempotencyKey: 'west-sandbox-refund-idem-001',
		amountMinor: 4999
	});
	assert(refund.status === 'refunded', `expected refunded status, got ${refund.status}`);

	const webhookPayload = JSON.stringify({
		eventId: 'west-sandbox-webhook-evt-001',
	...
	const webhookSignature = signWebhook(webhookPayload, process.env.WEST_PAYMENTS_WEBHOOK_SECRET || '');
	const webhook1 = await plugin.payment.verifyWebhook(ctx, {
		headers: { 'x-west-payments-signature': webhookSignature },
	...
	console.error('[payments-western-provider-sandbox-smoke] FAIL', error);
	process.exitCode = 1;
} finally {
	if (closeServer) {
		try {
