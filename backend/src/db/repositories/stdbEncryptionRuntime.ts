...
export function stdbEncryptionRows(key: string, query: string): StdbDecodedRow[] | null {
	if (!stdbEncryptionEnabled()) return null;
	try {
...
export function stdbEncryptionIngest(
	...
	if (!stdbEncryptionEnabled()) return false;
	try {
