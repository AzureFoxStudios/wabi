...
use super::{
    extract_user_id, is_admin_user, get_policy_row, upsert_policy, upsert_account_link,
    json_error, DEFAULT_WORKSPACE_ID,
    PaymentAccessPolicy, PaymentAccountLink, PaymentDonationConfig, PaymentUserBlock,
    AccountLinkInput, UserBlockInput, ListQuery, SaveAccessInput, SaveDonationInput,
};
...
pub async fn list_account_links(
    ...
    let links: Vec<PaymentAccountLink> = state.stdb.sql_query(&sql).await
        .map(|resp| {
            resp.decode_rows().iter()
                .filter_map(|row| row.get("row_json").and_then(|v| v.as_str()).and_then(|s| serde_json::from_str(s).ok()))
                .collect()
        })
        .unwrap_or_default();
...
pub async fn delete_account_link(
    ...
    let _ = state.stdb.ingest_event("payment", "delete_account_link", &json!({
        "userId": user_id,
        "pluginId": plugin_id,
...
pub async fn list_user_blocks(
    ...
    let sql = format!(
        ...
        crate::db::StdbClient::sanitize_sql(workspace_id), limit,
    );
    let blocks: Vec<PaymentUserBlock> = state.stdb.sql_query(&sql).await
        .map(|resp| {
            resp.decode_rows().iter()
                .filter_map(|row| row.get("row_json").and_then(|v| v.as_str()).and_then(|s| serde_json::from_str(s).ok()))
                .collect()
...
pub async fn create_user_block(
    ...
    let _ = state.stdb.ingest_event("payment", "upsert_user_block", &json!({
...
pub async fn clear_user_block(
    ...
    let _ = state.stdb.ingest_event("payment", "delete_user_block", &json!({
...
fn sanitize_access_policy(raw: &serde_json::Value) -> PaymentAccessPolicy {
    ...
    let obj = match raw.as_object() { Some(o) => o, None => return fallback };

    let enabled = obj.get("enabled").and_then(|v| v.as_bool()).unwrap_or(fallback.enabled);
    let allow_guest = obj.get("allowGuest").and_then(|v| v.as_bool()).unwrap_or(fallback.allow_guest);
    let allowed_role_names = obj.get("allowedRoleNames").and_then(|v| v.as_array())
        .map(|arr| {
            let mut roles: Vec<String> = arr.iter()
    ...
    PaymentAccessPolicy { enabled, allow_guest, allowed_role_names }
}
...
fn sanitize_donation_config(raw: &serde_json::Value) -> PaymentDonationConfig {
    ...
    let enabled = obj.get("enabled").and_then(|v| v.as_bool()).unwrap_or(fallback.enabled);
    let provider_plugin_id = obj.get("providerPluginId").and_then(|v| v.as_str())
        .filter(|s| !s.trim().is_empty()).map(|s| s.trim().chars().take(96).collect());
    let method_id = obj.get("methodId").and_then(|v| v.as_str())
        .filter(|s| !s.trim().is_empty()).map(|s| s.trim().chars().take(96).collect());
    let currency = obj.get("currency").and_then(|v| v.as_str())
        .filter(|s| s.len() == 3 && s.chars().all(|c| c.is_ascii_alphabetic()))
        .unwrap_or(&fallback.currency).to_uppercase();
    let country_code = obj.get("countryCode").and_then(|v| v.as_str())
        .filter(|s| s.len() == 2 && s.chars().all(|c| c.is_ascii_alphabetic()))
        .map(|s| s.to_uppercase());
    let suggested_amounts = obj.get("suggestedAmountsMinor").and_then(|v| v.as_array())
        .map(|arr| {
            let mut amounts: Vec<i64> = arr.iter().filter_map(|v| v.as_i64())
                .filter(|&n| n > 0 && n <= 1_000_000_000).collect();
            amounts.sort();
            amounts.dedup();
            amounts.truncate(8);
            if amounts.is_empty() { fallback.suggested_amounts_minor.clone() } else { amounts }
        })
        .unwrap_or_else(|| fallback.suggested_amounts_minor.clone());
    let headline = obj.get("headline").and_then(|v| v.as_str())
        .filter(|s| !s.trim().is_empty()).map(|s| s.trim().chars().take(120).collect())
        .unwrap_or(fallback.headline);
    let description = obj.get("description").and_then(|v| v.as_str())
        .filter(|s| !s.trim().is_empty()).map(|s| s.trim().chars().take(500).collect())
        .unwrap_or(fallback.description);

    PaymentDonationConfig {
        enabled, provider_plugin_id, method_id, currency, country_code,
        suggested_amounts_minor: suggested_amounts, headline, description,
    }
}
