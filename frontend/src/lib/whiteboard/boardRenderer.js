...
function isProtectedWhiteboardImage(src) {
    try {
...
async function loadProtectedImage(img, src) {
...
export function preloadImage(src) {
    ...
    if (isProtectedWhiteboardImage(src) && !img.src && !imageLoadCache.has(src)) {
        imageLoadCache.set(src, loadProtectedImage(img, src)
            .catch((error) => {
...
// Main render
// ---------------------------------------------------------------------------
export function renderElements(ctx, elements, viewport, layers = []) {
    ctx.save();
...
function drawArrowHead(ctx, x, y, angle, len, color) {
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.moveTo(x, y);
...
function renderText(ctx, el) {
    ...
    for (let i = 0; i < lines.length; i++) {
        let tx = el.x;
        if (te.textAlign === 'center')
            tx = el.x + el.width / 2;
        else if (te.textAlign === 'right')
            tx = el.x + el.width;
...
// Grid
// ---------------------------------------------------------------------------
export function renderGrid(ctx, viewport, w, h, gridSize) {
...
// Selection overlays
// ---------------------------------------------------------------------------
export function renderSelectionBox(ctx, bbox, viewport) {
...
export function renderHandles(ctx, handles) {
    const size = 8;
...
export function renderDrawPreview(ctx, previewEl, viewport) {
    ctx.save();
    ctx.scale(viewport.zoom, viewport.zoom);
    ctx.translate(-viewport.x, -viewport.y);
    ...
    switch (previewEl.type) {
        case 'stroke':
            renderStroke(ctx, previewEl);
            break;
        case 'line':
            renderLine(ctx, previewEl);
            break;
        case 'rect':
            renderRect(ctx, previewEl);
            break;
        case 'ellipse':
            renderEllipse(ctx, previewEl);
            break;
        case 'arrow':
            renderArrow(ctx, previewEl);
            break;
        case 'text':
            renderText(ctx, previewEl);
            break;
        case 'image':
            renderImage(ctx, previewEl);
            break;
    }
    ctx.restore();
}
export function renderSelectionRect(ctx, rect, viewport) {
...
export function renderRemoteCursors(ctx, cursors, viewport) {
