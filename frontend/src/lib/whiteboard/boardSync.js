...
import { joinWhiteboardChannel, leaveWhiteboard, saveWhiteboardSnapshot, sendWhiteboardPatch, sendWhiteboardCursor, subscribeWhiteboardEvents } from './boardSocket';
import { getChannelBoardId } from './boardTypes';
import { boardStore, isDirty, setPatchListener } from './boardStore';
import { fromTransportElement, toTransportElement } from './elementTypes';
import { currentUser } from '$lib/socket-manager';
// ---------------------------------------------------------------------------
// Patch emission
// ---------------------------------------------------------------------------
export function emitCreatePatch(boardId, el) {
    sendWhiteboardPatch(boardId, { op: 'create', element: toTransportElement(el) });
}
export function emitUpdatePatch(boardId, id, changes) {
    sendWhiteboardPatch(boardId, { op: 'update', id, changes });
}
export function emitDeletePatch(boardId, ids) {
    sendWhiteboardPatch(boardId, { op: 'delete', ids });
}
export function emitReorderPatch(boardId, id, dir) {
    sendWhiteboardPatch(boardId, { op: 'reorder', id, dir });
}
export function emitLayerCreatePatch(boardId, layer) {
    sendWhiteboardPatch(boardId, { op: 'layer:create', layer });
}
export function emitLayerUpdatePatch(boardId, id, changes) {
    sendWhiteboardPatch(boardId, { op: 'layer:update', id, changes });
}
export function emitLayerDeletePatch(boardId, id) {
    sendWhiteboardPatch(boardId, { op: 'layer:delete', id });
}
export function emitLayerReorderPatch(boardId, id, dir) {
    sendWhiteboardPatch(boardId, { op: 'layer:reorder', id, dir });
}
export function emitLayerSelectPatch(boardId, id) {
    sendWhiteboardPatch(boardId, { op: 'layer:select', id });
}
// ---------------------------------------------------------------------------
// Remote patch application
// ---------------------------------------------------------------------------
export function applyRemotePatch(payload) {
    ...
    switch (patch.op) {
        ...
        case 'layer:reorder': {
            if (typeof patch.id === 'string' && typeof patch.dir === 'string') {
                boardStore.reorderLayerSilent(patch.id, patch.dir);
            }
            break;
        }
        case 'layer:select': {
...
export function scheduleSnapshotSave(boardId) {
...
export function flushSnapshotSave(boardId) {
    const existing = snapshotTimers.get(boardId);
    if (existing) {
        clearTimeout(existing);
        snapshotTimers.delete(boardId);
    ...
    if (!doc.boardId || doc.boardId !== boardId)
        return;
    const transportDoc = {
        boardId: doc.boardId,
        version: doc.version,
...
export function cancelSnapshotSave(boardId) {
    const existing = snapshotTimers.get(boardId);
    if (existing) {
        clearTimeout(existing);
        snapshotTimers.delete(boardId);
...
export function broadcastCursor(boardId, data) {
    const now = Date.now();
...
export function createSyncSession(channelId, handlers = {}) {
    const boardId = getChannelBoardId(channelId);
    boardStore.setBoardId(boardId);
    const localUser = get(currentUser);
    const localStableId = typeof localUser?.dbUserId === 'number' ? `user-${localUser.dbUserId}` : (localUser?.id || '');
    ...
    setPatchListener((type, payload) => {
        switch (type) {
            case 'create':
                emitCreatePatch(boardId, payload);
                break;
            case 'update': {
                const p = payload;
