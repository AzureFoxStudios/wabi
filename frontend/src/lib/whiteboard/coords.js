...
// Viewport transforms
// ---------------------------------------------------------------------------
export function boardToScreen(bx, by, vp) {
    return { x: (bx - vp.x) * vp.zoom, y: (by - vp.y) * vp.zoom };
}
export function screenToBoard(sx, sy, vp) {
...
export function getSelectionBBox(elements) {
...
export function hitTestElement(el, px, py, tolerance) {
    if (el.type === 'stroke') {
        const pts = el.points;
    ...
    const pad = tolerance;
    return (px >= bbox.x - pad &&
        px <= bbox.x + bbox.width + pad &&
        py >= bbox.y - pad &&
        py <= bbox.y + bbox.height + pad);
}
export function pickElement(elements, px, py, tolerance, layers = []) {
    let best = null;
    let bestZ = -Infinity;
    for (const el of elements) {
        if (el.locked)
...
export function getSelectionHandles(bbox, vp, size) {
...
export function hitTestHandle(handles, sx, sy, size) {
    const half = size / 2;
