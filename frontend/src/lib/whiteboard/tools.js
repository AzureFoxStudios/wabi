...
import { pickElement, hitTestHandle, getSelectionHandles, getSelectionBBox, getElementBBox, normalizeRect } from './coords';
import { resolveWritableWhiteboardLayerId } from './layers';
// ---------------------------------------------------------------------------
// Helper: create base element from current style
// ---------------------------------------------------------------------------
function makeBase(style, type, x, y, elements) {
...
export function createPenTool() {
    return {
        ...
        onPointerDown(e) {
            ...
            const points = [{ x: e.boardX, y: e.boardY, pressure: e.pressure }];
            let preview = { ...base, points: [...points] };
            return {
                onPointerMove(e) {
...
function computeStrokeBBox(pts) {
    if (pts.length === 0)
...
// ---------------------------------------------------------------------------
function createShapeTool(toolType) {
    return {
        id: toolType,
        cursor: 'crosshair',
        onPointerDown(e) {
            ...
            return {
                onPointerMove(ev) {
                    ...
                    if (toolType === 'line' || toolType === 'arrow') {
                        preview = { ...base, width: w, height: h };
                    }
                    else {
                        const norm = normalizeRect(startX, startY, w, h);
                        preview = { ...base, x: norm.x, y: norm.y, width: norm.width, height: norm.height };
                    }
                },
                onPointerUp(ev) {
                    let w = ev.boardX - startX;
                    ...
                    let el;
                    if (toolType === 'line' || toolType === 'arrow') {
                        el = { ...base, width: w, height: h };
                    }
                    else {
                        const norm = normalizeRect(startX, startY, w, h);
                        el = { ...base, x: norm.x, y: norm.y, width: norm.width, height: norm.height };
                    }
                    boardStore.addElement(el);
                },
                getPreview() { return preview; },
...
let textPlacementCallback = null;
export function onTextPlacement(cb) {
...
function createMoveInteraction(startEvent) {
        ...
        onPointerMove(e) {
            ...
            for (const id of state.selection) {
                ...
                if (!el)
                    continue;
                if (el.type === 'stroke') {
                    const pts = el.points.map((p) => ({ ...p, x: p.x + dx, y: p.y + dy }));
                    boardStore.updateElement(id, { x: el.x + dx, y: el.y + dy, points: pts }, { recordHistory: false });
                }
                else {
...
function createResizeInteraction(selectedEls, origBBox, handle, startEvent) {
    ...
    const origPositions = selectedEls.map((el) => ({
        ...
        originalPoints: el.type === 'stroke' ? el.points.map((point) => ({ ...point })) : null,
        ...
        onPointerMove(e) {
            ...
            for (const orig of origPositions) {
                ...
                if (orig.type === 'stroke' && orig.originalPoints) {
                    ...
                    boardStore.updateElement(orig.id, {
                        x: nextX,
                        y: nextY,
                        width: nextWidth,
                        height: nextHeight,
...
function createRubberBandInteraction(startEvent) {
    const startBX = startEvent.boardX;
    const startBY = startEvent.boardY;
    let rect = null;
    return {
        onPointerMove(e) {
            ...
            for (const el of state.elements) {
                ...
                if (el.locked || layer?.locked || layer?.visible === false)
                    continue;
                const bb = getElementBBox(el);
                if (bb.x >= rect.x && bb.y >= rect.y &&
...
// Tool registry
// ---------------------------------------------------------------------------
export function getToolHandler(toolType) {
    switch (toolType) {
