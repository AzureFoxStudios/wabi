...
const BIP39_WORDLIST: readonly string[] = [
  ...
  'laptop','large','later','latin','laugh','laundry','lava','lawn','lawsuit','layer',
...
const HKDF_INFO = 'wabi-dm-identity'

async function entropyToMnemonic(entropy: Uint8Array): Promise<string[]> {
  const ENT = entropy.length * 8
  const CS = ENT / 32
  const hash = new Uint8Array(await crypto.subtle.digest('SHA-256', toAB(entropy)))
  const bits: number[] = []
...
async function mnemonicToEntropy(words: string[]): Promise<Uint8Array> {
  const bits: number[] = []
  ...
  const ENT_BITS = bits.length - CS
  const entropyBytes = new Uint8Array(ENT_BITS / 8)
  for (let i = 0; i < ENT_BITS / 8; i++) {
    let byte = 0
  ...
  for (let i = 0; i < CS; i++) {
    if (bits[ENT_BITS + i] !== ((hash[i >> 3] >> (7 - (i % 8))) & 1)) {
...
export async function mnemonicToSeed(mnemonic: string[], passphrase: string = ''): Promise<ArrayBuffer> {
  const encoder = new TextEncoder()
  const mnemonicStr = mnemonic.join(' ')
  const keyMaterial = await crypto.subtle.importKey(
    'raw', encoder.encode(mnemonicStr), 'PBKDF2', false, ['deriveBits']
  )
  return crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: toAB(encoder.encode('mnemonic' + passphrase)),
...
async function hkdfDeriveBytes(seed: ArrayBuffer, salt: Uint8Array, info: string, length: number): Promise<ArrayBuffer> {
  const hkdfKey = await crypto.subtle.importKey('raw', seed, 'HKDF', false, ['deriveBits'])
  return crypto.subtle.deriveBits(
    { name: 'HKDF', hash: 'SHA-256', salt: toAB(salt), info: new TextEncoder().encode(info) },
    hkdfKey,
    length
  )
}

export async function mnemonicToKeypair(mnemonic: string[]): Promise<{ privateKey: CryptoKey; publicKey: CryptoKey; publicKeyB64: string }> {
  const seed = await mnemonicToSeed(mnemonic)
  const rawPrivate = new Uint8Array(await hkdfDeriveBytes(seed, new Uint8Array(16), HKDF_INFO, 256))
  const pubBytes = computePublicKey(rawPrivate)
  const pubB64 = base64url(toAB(pubBytes))
  const privB64 = base64url(toAB(rawPrivate))
  const privateKey = await crypto.subtle.importKey(
    'jwk',
    { kty: 'OKP', crv: 'X25519', d: privB64, x: pubB64, key_ops: ['deriveBits', 'deriveKey'] },
    { name: 'X25519' },
    false,
  ...
  const publicKey = await crypto.subtle.importKey(
    'jwk',
    { kty: 'OKP', crv: 'X25519', x: pubB64, key_ops: [] },
    { name: 'X25519' },
    false,
    []
  )
  return { privateKey, publicKey, publicKeyB64: pubB64 }
}

export async function loadIdentity(mnemonic: string[]): Promise<{ privateKey: CryptoKey; publicKey: CryptoKey; publicKeyB64: string }> {
...
export function createVerificationChallenge(mnemonic: string[]): { indices: number[]; expected: string[] } {
  const indices: number[] = []
  const used = new Set<number>()
  while (indices.length < 3) {
    const idx = Math.floor(Math.random() * mnemonic.length)
    if (!used.has(idx)) {
      used.add(idx)
      indices.push(idx)
    }
  }
  return { indices, expected: indices.map((i) => mnemonic[i]!) }
}
