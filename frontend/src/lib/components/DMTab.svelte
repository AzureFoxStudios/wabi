...
<style>
	...
	.dm-new-user:hover { background: var(--surface-hover); }
	...
	.dm-open-mode-badge {
		...
		border: 1px solid var(--shadow-lg, var(--surface-modal-overlay, rgba(0, 0, 0, 0.45)));
		display: flex;
		align-items: center;
		justify-content: center;
	...
	.dm-conv-status-dot {
		...
		height: 14px;
		border-radius: 50%;
		border: 3px solid var(--surface-app, #1a1a2e);
	...
	.dm-conv-status-dot.active {
		background: var(--status-online, #22c55e);
		box-shadow: 0 0 0 0 rgba(var(--status-online-rgb, 34, 197, 94), 0.45);
		animation: status-pulse-online 2.5s infinite ease-in-out;
	}

	.dm-conv-status-dot.away { background: var(--status-away, #f59e0b); }
	.dm-conv-status-dot.busy { background: var(--status-busy, #ef4444); }

	@keyframes status-pulse-online {
		0%, 100% { box-shadow: 0 0 0 0 rgba(var(--status-online-rgb, 34, 197, 94), 0.45); }
		50% { box-shadow: 0 0 0 5px rgba(var(--status-online-rgb, 34, 197, 94), 0); }
	}
