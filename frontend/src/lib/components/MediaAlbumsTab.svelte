<script lang="ts">
	...
	function navigateAlbumViewer(direction: 'prev' | 'next'): void {
		if (albumViewerItems.length <= 1) return;
		albumViewerIndex =
			direction === 'prev'
				? (albumViewerIndex - 1 + albumViewerItems.length) % albumViewerItems.length
				: (albumViewerIndex + 1) % albumViewerItems.length;
	}
	...
	<div class="screenshot-pipe-panel">
		<div class="screenshot-pipe-panel__header">
			...
			<div class="screenshot-pipe-panel__actions">
				<button
					type="button"
					class:active={screenshotPipeEnabled}
					on:click={() => {
						screenshotPipeEnabled = !screenshotPipeEnabled;
						persistScreenshotPipeSettings();
					}}
					disabled={!scopeId}
				>
					{screenshotPipeEnabled ? 'Enabled' : 'Disabled'}
				</button>
				<button
					type="button"
					on:click={() => void scanGameScreenshotPipeNow()}
					disabled={!scopeId || !screenshotPipeEnabled || !screenshotPipeTargetAlbumId || isScanningScreenshotPipe}
				>
					{isScanningScreenshotPipe ? 'Scanning...' : 'Scan now'}
				</button>
			</div>
		</div>
		<div class="screenshot-pipe-panel__row">
			<input
				type="text"
				bind:value={screenshotPipeFolderPath}
				placeholder="FFXIV screenshot folder path"
				on:change={() => persistScreenshotPipeSettings()}
				disabled={!scopeId}
			/>
			<button
				type="button"
				on:click={() => void useDetectedScreenshotFolder()}
				disabled={!scopeId || isLoadingScreenshotPipeCandidates}
			>
				{isLoadingScreenshotPipeCandidates ? 'Detecting...' : 'Detect folder'}
			</button>
		</div>
		<div class="screenshot-pipe-panel__row">
			<button
			...
			<button
				type="button"
				on:click={clearScreenshotPipeTarget}
				disabled={!scopeKey || screenshotPipeTargetAlbumId === null}
			>
				Clear target
			</button>
			</div>
			<div class="screenshot-pipe-panel__meta">
				<span>
					{#if screenshotPipeTargetAlbumId}
						Target album: {albums.find((album) => album.id === screenshotPipeTargetAlbumId)?.name || `#${screenshotPipeTargetAlbumId}`}
					{:else}
						No target album set for this scope.
					{/if}
				</span>
			<span>{screenshotPipeFolderPath || 'No folder selected yet.'}</span>
		</div>
		{#if screenshotPipeCandidates.length > 0}
			<div class="screenshot-pipe-candidates">
				{#each screenshotPipeCandidates.filter((candidate) => candidate.exists).slice(0, 5) as candidate}
					<button
						type="button"
						class="screenshot-pipe-candidate"
						on:click={() => {
							screenshotPipeFolderPath = candidate.path;
							persistScreenshotPipeSettings();
						}}
					>
						{candidate.label}
		...
		{/if}
		{#if screenshotPipeStatusMessage}
			<div class="success-banner">{screenshotPipeStatusMessage}</div>
		{/if}
		{#if screenshotPipeErrorMessage}
			<div class="error-banner">{screenshotPipeErrorMessage}</div>
		{/if}
	</div>
			...
			>
				<div class="album-viewer-header">
					<div class="album-viewer-copy">
						...
						<span>{albumViewerIndex + 1} / {albumViewerItems.length} &middot; {albumViewerCurrentItem.attachmentName}</span>
					</div>
					<div class="album-viewer-actions">
						<a
							class="album-viewer-action"
							href={resolveAlbumAssetUrl(albumViewerCurrentItem.attachmentUrl)}
							target="_blank"
				...
				<div class="album-viewer-stage">
					...
					{:else}
						...
						/>
					{/if}
					{#if albumViewerItems.length > 1}
						<button type="button" class="album-viewer-nav album-viewer-nav-prev" on:click={() => navigateAlbumViewer('prev')} aria-label="Previous album item">
							&lt;
						</button>
						<button type="button" class="album-viewer-nav album-viewer-nav-next" on:click={() => navigateAlbumViewer('next')} aria-label="Next album item">
							&gt;
						</button>
					{/if}
				</div>
				{#if albumViewerCurrentItem.caption}
					<div class="album-viewer-caption">{albumViewerCurrentItem.caption}</div>
				{/if}
...
<style>
	...
	.screenshot-pipe-panel {
		...
		border-radius: 14px;
		background:
			linear-gradient(135deg, rgba(var(--accent-rgb), 0.12), rgba(var(--accent-rgb), 0.03)),
			var(--surface-base);
	}

	.screenshot-pipe-panel__header {
		display: flex;
		align-items: center;
		justify-content: space-between;
		gap: 0.75rem;
	...
	.screenshot-pipe-panel__copy {
		display: grid;
		gap: 0.12rem;
	}

	.screenshot-pipe-panel__copy strong {
		font-size: 0.8rem;
	}

	.screenshot-pipe-panel__copy span,
	.screenshot-pipe-panel__meta {
		font-size: 0.72rem;
		color: var(--text-secondary);
		line-height: 1.4;
	}

	.screenshot-pipe-panel__actions,
	.screenshot-pipe-panel__row {
		display: flex;
		gap: 0.45rem;
		flex-wrap: wrap;
	}

	.screenshot-pipe-panel__row input {
		flex: 1 1 18rem;
	}

	.screenshot-pipe-panel__actions button,
	.screenshot-pipe-panel__row button,
	.screenshot-pipe-candidate {
	...
	.screenshot-pipe-panel__actions button.active {
		border-color: rgba(var(--accent-rgb), 0.8);
		background: rgba(var(--accent-rgb), 0.2);
	}

	.screenshot-pipe-panel__actions button:disabled,
	.screenshot-pipe-panel__row button:disabled,
	.screenshot-pipe-candidate:disabled {
		opacity: 0.6;
		cursor: not-allowed;
	}

	.screenshot-pipe-candidates {
		display: flex;
		flex-wrap: wrap;
		gap: 0.35rem;
	}
	...
	.error-banner {
		border: 1px solid rgba(var(--color-danger-rgb, 220, 38, 38), 0.45);
	...
	.success-banner {
		border: 1px solid rgba(var(--status-online-rgb, 34, 197, 94), 0.35);
		background: rgba(var(--status-online-rgb, 34, 197, 94), 0.12);
		color: var(--accent-success-soft, #bbf7d0);
		padding: 0.45rem 0.55rem;
		border-radius: 8px;
		font-size: 0.76rem;
	}
	...
	.album-card.uploading {
		border-color: rgba(var(--status-online-rgb, 34, 197, 94), 0.45);
		background: rgba(var(--status-online-rgb, 34, 197, 94), 0.12);
		box-shadow: 0 0 0 1px rgba(var(--status-online-rgb, 34, 197, 94), 0.14);
	}
	...
	.album-quick-btn--danger:hover {
		color: var(--accent-danger-soft, #fca5a5);
		border-color: rgba(var(--color-danger-rgb, 239, 68, 68), 0.35);
	}
	...
	.album-card-placeholder {
		...
		background: rgba(var(--text-inverse-rgb, 255, 255, 255), 0.03);
		font-size: 0.72rem;
		color: var(--text-secondary);
		text-align: center;
	}
	...
	.album-viewer-backdrop {
		position: fixed;
		inset: 0;
		z-index: var(--z-lightbox, 12000);
		display: grid;
		place-items: center;
		padding: 1rem;
		background:
			radial-gradient(circle at 20% 12%, rgba(var(--accent-rgb), 0.2), transparent 34%),
			rgba(var(--surface-app-rgb, 2, 6, 23), 0.86);
	...
	.album-viewer-modal {
		...
		background:
			linear-gradient(180deg, rgba(var(--surface-app-rgb, 17, 24, 39), 0.98), rgba(var(--surface-app-rgb, 15, 23, 42), 0.96)),
			rgba(var(--surface-app-rgb, 15, 23, 42), 0.96);
		box-shadow: 0 28px 60px rgba(var(--surface-app-rgb, 2, 6, 23), 0.42);
	}
	...
	.album-viewer-action,
	.album-viewer-close {
		border: 1px solid rgba(var(--text-muted-rgb, 148, 163, 184), 0.28);
	...
	.album-viewer-stage {
		...
		overflow: hidden;
		background:
			radial-gradient(circle at top, rgba(var(--text-inverse-rgb, 255, 255, 255), 0.1), transparent 45%),
			rgba(var(--surface-app-rgb, 15, 23, 42), 0.92);
	...
	.album-viewer-nav:hover {
		background: rgba(var(--text-inverse-rgb, 255, 255, 255), 0.14);
	}

	.album-viewer-nav-prev {
		left: 0.75rem;
	}

	.album-viewer-nav-next {
		right: 0.75rem;
	}
	...
	.album-viewer-thumb {
		...
		padding: 0;
		overflow: hidden;
		background: rgba(var(--text-inverse-rgb, 255, 255, 255), 0.04);
		cursor: pointer;
	}
	...
	.album-quick-btn--danger {
		border-color: rgba(var(--color-danger-rgb, 220, 38, 38), 0.45);
		color: var(--accent-danger-soft, #fecaca);
	...
	.album-context-menu {
		...
		border: 1px solid var(--border-subtle);
		background: color-mix(in srgb, var(--surface-app) 92%, black 8%);
		box-shadow: 0 18px 44px rgba(var(--surface-app-rgb, 15, 23, 42), 0.28);
	}
	...
	.danger-btn {
		border: 1px solid rgba(var(--color-danger-rgb, 220, 38, 38), 0.6);
		background: rgba(var(--color-danger-rgb, 220, 38, 38), 0.14);
		color: var(--accent-danger-soft, #fecaca);
		border-radius: 8px;
	...
	.danger-btn:hover {
		background: rgba(var(--color-danger-rgb, 220, 38, 38), 0.22);
	}
	...
	.album-preview-chip {
		...
		border: 1px solid var(--border-subtle);
		background: rgba(var(--text-inverse-rgb, 255, 255, 255), 0.04);
		box-shadow: 0 10px 24px rgba(var(--surface-app-rgb, 15, 23, 42), 0.08);
		padding: 0;
		cursor: pointer;
	}
	...
	button.item-preview:hover,
	.album-preview-chip:hover {
	...
	.item-preview-fallback {
		...
		padding: 0.4rem;
		background:
			linear-gradient(135deg, rgba(var(--accent-rgb), 0.18), rgba(var(--text-muted-rgb, 148, 163, 184), 0.12)),
			rgba(var(--text-inverse-rgb, 255, 255, 255), 0.04);
		color: var(--text-secondary);
		font-size: 0.72rem;
		font-weight: 700;
