...
{#if visible && message}
	<div class="backdrop" role="presentation" onclick={(event) => { if (event.currentTarget === event.target) onClose(); }}>
		<section class="dialog" role="dialog" aria-modal="true" aria-labelledby="report-title">
			{#if sent}
				<div class="success"><span class="mark">✓</span><h2 id="report-title">Report sent</h2><p>Server staff can review the message snapshot and your note. Your report does not automatically punish anyone.</p><button onclick={onClose}>Done</button></div>
			{:else}
				...
				<p class="privacy">Wabi sends a server-side snapshot of this message to this server’s staff. It does not send the report to a central Wabi moderation service.</p>
				{#if error}<div class="error" role="alert">{error}</div>{/if}
				<footer><button class="cancel" onclick={onClose}>Cancel</button><button class="submit" onclick={submit} disabled={busy}>{busy ? 'Sending…' : 'Send report'}</button></footer>
			{/if}
		</section>
	</div>
{/if}
