<script lang="ts">
	...
	async function handleRegister() {
		...
		try {
			...
			if (result.user.id) {
				setStoredDbUserId(result.user.id);
	...
	onMount(() => {
		...
		if (isTauri) {
			// Desktop app: don't pre-fill a server — user chooses their own on first run.
			serverDomain = '';
...
<style>
	.login-container {
		...
		--launch-accent: var(--accent-primary);
		--launch-text: var(--text-heading);
	...
	.logo {
		...
		margin-bottom: 2rem;
		filter: invert(1) drop-shadow(0 4px 12px var(--surface-modal-overlay, var(--surface-modal-overlay, rgba(0, 0, 0, 0.4))));
	...
	.tab-btn.active {
		color: var(--launch-accent, var(--accent-primary));
		border-bottom-color: var(--launch-accent, var(--accent-primary));
	}
	...
	.qr-btn:hover {
		border-color: var(--launch-accent, var(--accent-primary));
		color: var(--launch-accent, var(--accent-primary));
