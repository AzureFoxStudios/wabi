<script lang="ts">
	...
	/** Bound: set by messages pane on reply click; cleared here on send/cancel */
	export let replyingTo: Message | null = null;
	export let composerVisible = true;
	export let isTextareaFocused = false;
	/** Called when the user submits a /command that may need Chat.svelte context */
	...
	$: composerInputMaxLength = splitLargeMessagesEnabled
		? composerSettings.splitLargeMessagesInputMaxLength
	...
	$: {
		const p = previewUnicodeEmojiConversion(messageInput, $emojis as unknown as Emoji[]);
		unicodeComposerPreview = p.convertedText;
		unicodeComposerPreviewTokens = p.convertedTokens;
	}
	$: {
		const p = previewUnicodeEmojiConversion(gifCaptionInput, $emojis as unknown as Emoji[]);
		unicodeGifCaptionPreview = p.convertedText;
		unicodeGifCaptionPreviewTokens = p.convertedTokens;
	}
	...
	export async function receiveFiles(files: File[], mode: 'replace' | 'append'): Promise<void> {
		const prepared = await prepareIncomingFiles(files);
		if (prepared.length > 0) applySelectedFiles(prepared, mode);
	}

	export function insertQuickMention(username: string): void {
		const token = `@${username}`;
		const needsSpace = messageInput.length > 0 && !/\s$/.test(messageInput);
		const next = needsSpace ? `${messageInput} ${token} ` : `${token} `;
	...
	export function startEditingLastMessage(): void {
	...
	function autoResizeTextarea() {
		...
		textareaElement.style.height = `${Math.min(textareaElement.scrollHeight, COMPOSER_MAX_HEIGHT)}px`;
	}

	function handleInput() {
		autoResizeTextarea();
		const now = Date.now();
		if (now - lastTypingEmit >= TYPING_THROTTLE_MS) { sendTyping(true, $currentChannel); lastTypingEmit = now; }
		if (typingTimeout) clearTimeout(typingTimeout);
		typingTimeout = setTimeout(() => sendTyping(false, $currentChannel), 1000) as unknown as number;
	}
	...
	function handleInputChange() {
		...
		if (messageInput.startsWith('/')) {
			...
			if (!$placeRegistry.length) void loadPlaceRegistry();
			const result = computeMentionSuggestions(messageInput, caret, $users as User[], $currentUser?.id, $placeRegistry);
			...
			else showMentionSuggestions = false;
		}
	}
	...
	async function handlePaste(e: ClipboardEvent) {
		...
		for (let i = 0; i < items.length; i++) {
			const item = items[i];
			if (item.kind === 'file') { const f = item.getAsFile(); if (f) { e.preventDefault(); files.push(f); } }
		}
	...
	function resolveOutgoingAttachmentCaption(): string | null {
		...
		const src = gifCaptionerDedicatedCaptionFieldEnabled ? gifCaptionInput : messageInput;
		return processAttachmentCaption(src, { maxLength: MAX_GIF_CAPTION_LENGTH, writeUpperCaseEnabled, unicodeEmojisEnabled, emojis: $emojis as unknown as Emoji[] });
	}
	...
	function handleSubmit() {
		...
		if (editingMessage) {
			...
			if (!burst.allowed) {
				...
				sendCooldownTimer = setTimeout(() => { sendCooldownUntil = 0; sendCooldownMessage = ''; sendCooldownTimer = null; }, SEND_BURST_COOLDOWN_MS);
				return;
			}
			...
			if (splitLargeMessagesEnabled && processed.text.length > splitLargeMessagesChunkSize) {
				...
				for (const [i, chunk] of chunks.entries()) {
					sendChatMessage($currentChannel, chunk, 'text', { replyTo: i === 0 ? replyingTo?.id : undefined, isSpoiler: markAsSpoiler, entities: chunkEntities[i] });
				}
			} else {
				const kind = detectMessageKind(processed.text, $emojis as unknown as Emoji[]);
				if (kind.type === 'emoji') {
					sendChatMessage($currentChannel, processed.text, 'emoji', { emojiUrl: kind.emojiUrl, emojiName: kind.emojiName, replyTo: replyingTo?.id, isSpoiler: markAsSpoiler });
				} else {
					sendChatMessage($currentChannel, processed.text, 'text', { replyTo: replyingTo?.id, isSpoiler: markAsSpoiler, entities: normalizedEntities });
				}
			}
			replyingTo = null;
		}
	...
	function handleGifSelect(event: CustomEvent<string>) {
		...
		else { messageInput = ''; resetComposerEntityState(); }
		showMentionSuggestions = false;
	...
	function clearFilePreviews(): void {
		videoCompressionController?.clearCompressionMetadata(filePreviews.map(i => i.file));
		for (const item of filePreviews) revokePreviewUrl(item.preview);
		filePreviews = [];
		selectedFiles = [];
	...
	function applySelectedFiles(files: File[], mode: 'replace' | 'append'): void {
		if (mode === 'replace') { clearFilePreviews(); selectedFiles = files; filePreviews = buildPreviewEntries(files); }
		else { selectedFiles = [...selectedFiles, ...files]; filePreviews = [...filePreviews, ...buildPreviewEntries(files)]; }
		const b = enforcePreviewBudget(filePreviews, selectedFiles, MAX_FILE_PREVIEW_IMAGES);
		filePreviews = b.previews;
		selectedFiles = b.files;
	}

	async function prepareIncomingFiles(files: File[]): Promise<File[]> {
		for (const file of files) {
			if (file.size > MAX_UPLOAD_FILE_BYTES) { alert(`File too large! Maximum size is 1GB per file. "${file.name}" is ${formatFileMb(file.size)}MB`); return []; }
		}
		const prepared: File[] = [];
		for (const file of files) {
			const candidate = videoCompressionController ? await videoCompressionController.maybeCompressVideoFile(file) : file;
	...
	async function handleFileSelect(event: Event) {
		...
		const prepared = await prepareIncomingFiles(files);
		if (prepared.length > 0) applySelectedFiles(prepared, 'replace');
		input.value = '';
	}
	...
	function removeFile(index: number) {
		...
		if (removed?.file) videoCompressionController?.deleteCompressionMetadata(removed.file);
	...
	function handleAlbumUploadToggle(checked: boolean): void {
		...
		if (!checked) { uploadAlbumName = ''; return; }
	...
	async function uploadSelectedFiles() {
		...
		try {
			...
			const spec = await orchestrateUpload({
				files: selectedFiles, channelId: $currentChannel,
				channelType: activeChannel?.type || 'channel',
				dmChannelId: activeChannel?.type === 'dm' ? activeChannel.id : undefined,
				dmOtherDbUserId, authToken,
				messageInput: messageInput.trim(),
				replyToId: replyingTo?.id, markAsSpoiler, captionEntities,
				createAlbum: createAlbumFromUpload, albumName: uploadAlbumName,
				albumScopeType: albumScope?.scopeType ?? null, albumScopeId: albumScope?.scopeId ?? null,
				getCompressionMetadata: f => videoCompressionController?.getCompressionMetadata(f),
	...
	async function handlePhotoCapture(event: CustomEvent<Blob>) {
		...
		if (file.size > 10 * 1024 * 1024) { alert('Photo too large (max 10MB). Please try again.'); return; }
		clearFilePreviews(); selectedFiles = [file]; filePreviews = buildPreviewEntries([file]);
		await uploadSelectedFiles();
		showCameraCapture = false; showMediaMenu = false;
	}
	...
	async function handleAudioSend(event: CustomEvent<Blob>) {
		...
		if (file.size > 10 * 1024 * 1024) { alert('Audio too large (max 10MB). Please try again.'); return; }
		clearFilePreviews(); selectedFiles = [file]; filePreviews = buildPreviewEntries([file]);
		await uploadSelectedFiles();
		showAudioRecorder = false; showMediaMenu = false;
	}
	...
	function ensureEmojiPickerLoaded(): void {
		...
		emojiPickerLoadPromise = import('../EmojiPicker.svelte')
			.then(mod => { EmojiPickerComponent = mod.default; })
			.catch(err => { console.error('Failed to load EmojiPicker:', err); })
			.finally(() => { emojiPickerLoadPromise = null; });
	}
...
<div class="input-wrapper" class:hidden={$layoutStore.isMobile && !composerVisible}>
	{#if showMentionSuggestions && mentionSuggestions.length > 0}
		<MentionSuggestions
			...
			bind:container={mentionMenuContainer}
			onApply={applyMentionSuggestion}
		/>
	{/if}
	{#if filePreviews.length > 0 && !isUploading}
		<FileUploadPreview
			...
			{createAlbumFromUpload}
			bind:uploadAlbumName
			buildDefaultUploadAlbumName={() => buildDefaultUploadAlbumName(
				$channels.find(ch => ch.id === $currentChannel)?.name || $currentChannel,
				messageInput
			)}
			onAlbumUploadToggle={handleAlbumUploadToggle}
			onCancelUpload={clearFilePreviews}
	...
	{#if sendCooldownMessage}
		<div class="composer-rate-limit-notice" role="status" aria-live="polite">{sendCooldownMessage}</div>
	{/if}
	...
	<div class="input-container">
		...
		<div class="input-buttons-left">
			<div class="media-menu-container" bind:this={mediaMenuContainer}>
				<button class="input-icon-button" on:click|stopPropagation={() => { showMediaMenu = !showMediaMenu; if (showMediaMenu) showEmojiPicker = false; }} title={$_('chat.compose.add_media')}>
				...
				{#if showMediaMenu}
					<div class="media-menu">
						<button class="media-menu-item" on:click={() => { showMediaMenu = false; fileInput?.click(); }}>{$_('chat.compose.upload_file')}</button>
						{#if supportsMediaCapture()}
							<button class="media-menu-item" on:click={() => { showMediaMenu = false; showCameraCapture = true; }}>{$_('chat.compose.take_photo')}</button>
							<button class="media-menu-item" on:click={() => { showMediaMenu = false; showAudioRecorder = true; }}>{$_('chat.compose.record_audio')}</button>
						{/if}
					</div>
				{/if}
			</div>
			<button class="input-icon-button" on:click={() => onOpenPaymentSheet()} title="Create payment request" disabled={!paymentButtonEnabled}>
			...
			{#if isDMChannel}
				<button class="input-icon-button" on:click={onOpenManualCash} title="Record manual cash trade" disabled={!paymentButtonEnabled}>
		...
		<textarea
			...
			on:input={() => { handleInput(); handleInputChange(); }}
			on:keydown={handleKeyDown}
			on:focus={() => { isTextareaFocused = true; composerVisible = true; }}
		...
		{/if}
		<button bind:this={emojiPickerButton} class="input-icon-button" on:click|stopPropagation={() => { showEmojiPicker = !showEmojiPicker; if (showEmojiPicker) { ensureEmojiPickerLoaded(); showMediaMenu = false; } }} title={$_('chat.compose.add_emoji')}>
		...
		<button class="send-button" on:click={handleSubmit} disabled={(selectedFiles.length === 0 && !messageInput.trim()) || sendCooldownUntil > Date.now() || isUploading} title={selectedFiles.length > 0 ? 'Send selected media' : $_('chat.compose.send_message')}>
	...
	{#if unicodeEmojisEnabled && unicodeComposerPreviewTokens > 0 && unicodeComposerPreview !== messageInput}
		<div class="unicode-conversion-hint">Unicode preview: {unicodeComposerPreview}</div>
	{/if}

	{#if gifCaptionerEnabled && gifCaptionerDedicatedCaptionFieldEnabled}
		<div class="gif-caption-draft-row">
			<input type="text" class="gif-caption-draft-input input input-sm" bind:value={gifCaptionInput} maxlength={MAX_GIF_CAPTION_LENGTH} placeholder="GIF caption (used when sending from GIF picker)" />
			<span class="gif-caption-draft-count" class:warn={gifCaptionDraftWarn}>{gifCaptionDraftLength}/{MAX_GIF_CAPTION_LENGTH}</span>
		</div>
		{#if unicodeEmojisEnabled && unicodeGifCaptionPreviewTokens > 0 && unicodeGifCaptionPreview !== gifCaptionInput}
			<div class="unicode-conversion-hint">GIF caption preview: {unicodeGifCaptionPreview}</div>
		{/if}
	{:else if gifCaptionerEnabled && showEmojiPicker}
		<div class="gif-caption-hint">GIF caption uses composer text (max {MAX_GIF_CAPTION_LENGTH} characters).</div>
	{/if}
</div>
