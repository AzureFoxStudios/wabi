...
<div class="infra-center">
	...
	{#if !error}<div class="summary"><div><strong>{relays.filter(r=>health(r)==='alive').length}</strong><span>Alive</span></div><div><strong>{relays.filter(r=>health(r)==='stale').length}</strong><span>Stale / offline</span></div><div><strong>{relays.filter(r=>health(r)==='pending').length}</strong><span>Awaiting approval</span></div><div><strong>{relays.length}</strong><span>Known nodes</span></div></div>{/if}
	{#if error}
		<section class="infra-read-error" role="alert">
			<strong>Infrastructure unavailable</strong>
			<p>The node roster could not be loaded. Refresh to try again.</p>
			<small>Server response: {error}</small>
		</section>
	{/if}
	{#if loading && !relays.length}<p class="empty">Reading node roster…</p>{:else if !relays.length && !error}<section class="empty"><strong>No auxiliary nodes registered.</strong><span>The origin server can still operate without a relay/helper.</span></section>{:else}
