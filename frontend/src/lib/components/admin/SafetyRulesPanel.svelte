	...
	{#if loading}<p>Loading safety rules…</p>{:else}
		...
		<div class="rules">
			{#each policy.rules as rule (rule.id)}
				...
				{#if rule.action === 'flag'}<p class="hint">Flagging creates a moderation item without automatically punishing the sender.</p>{/if}
			</article>
			{/each}
