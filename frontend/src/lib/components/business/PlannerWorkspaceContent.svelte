<script lang="ts">
	...
	import { capturePlannerSession, flushBusinessStorage, downloadPlannerData } from '$lib/business/deviceStorage';
	...
	function handleExport(): void {
		...
		try {
			...
			downloadPlannerData({ ...getBusinessDataSnapshot(), exportedAt: new Date().toISOString(), version: '1.0', sourceScope: captured.session.scope });
		} catch (error) { showToast(error instanceof Error ? error.message : 'Export failed', 'error'); }
	}
	...
	async function handleImportFile(event: Event): Promise<void> {
		...
		try {
			...
			const incoming = parsePlannerBackup(await file.text(), captured.session.scope);
			if (!captured.isCurrent()) throw new Error('Your Planner account changed. Select the import again.');
			const result = mergePlannerBackup(getBusinessDataSnapshot(), incoming);
			applyBusinessDataSnapshot(result.data);
			const saved = await flushBusinessStorage();
			if (!captured.isCurrent()) return;
			showToast(saved ? `Added ${result.added} items; kept ${result.skipped} existing items.` : 'Import is in your draft but could not be saved. Export the draft or retry saving.', saved ? 'info' : 'error');
		} catch (error) { showToast(error instanceof Error ? error.message : 'Import failed — check the file format', 'error'); }
	}
				...
				{#if overflowMenuOpen}
					<div class="planner-overflow-menu" role="menu" aria-label="Planner options">
						...
						<button type="button" role="menuitem" on:click={handleImportClick}>
							...
							<span class="menu-hint">Add items without replacing existing work</span>
						</button>
					</div>
				{/if}
			</div>
