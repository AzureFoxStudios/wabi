				...
				{#each filteredUsers as user (getDmDirectoryKey(user))}
					<button class="user-item" on:click={() => toggleUser(user)}>
						<div class="user-avatar-container">
							...
							{:else}
								<div class="user-avatar-placeholder" style="--avatar-color: {user.roleColor || user.color}">
...
<style>
	...
	.modal {
		...
		box-shadow: -4px 0 12px var(--shadow-sm, var(--shadow-md, var(--shadow-lg, rgba(0, 0, 0, 0.3))));
	}
