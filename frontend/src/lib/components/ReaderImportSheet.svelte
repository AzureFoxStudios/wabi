...
<dialog class="reader-import-dialog" use:openDialog aria-label="Import into Reader" oncancel={onClose}>
	<form onsubmit={(event) => { event.preventDefault(); if (importContent.trim()) onSubmit(); }}>
		...
		<div class="reader-import-grid">
			<label class="reader-field">Title<input type="text" bind:value={importTitle} maxlength="240" placeholder={importFormat === 'code' ? 'Filename, e.g. app.ts' : 'Document title'} /></label>
			<label class="reader-field">Format<select bind:value={importFormat}><option value="text">Plain text</option><option value="markdown">Markdown</option><option value="html">HTML</option><option value="code">Source code</option></select></label>
		</div>
		<label class="reader-field">Content<textarea bind:value={importContent} rows="12" placeholder={importFormat === 'code' ? 'Paste source code here. Use a filename with an extension to select syntax highlighting.' : 'Paste an article, chapter, essay, or notes here.'}></textarea></label>
