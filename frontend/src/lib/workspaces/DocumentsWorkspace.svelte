<script lang="ts">
    ...
    let scope=$state<Scope|null>(null);let active=$state<ArtifactSession|null>(null);let error=$state('');let loading=$state(true);let initialShare=$state<'snapshot'|'live'|undefined>();let preview=$state(true);let tick=$state(0);let stop:()=>void=()=>{};let generation=0;
    ...
    onMount(()=>{let unsub=()=>{};void run(async()=>{scope=await captureScope();unsub=targets.subscribe(value=>{if(value.documents)void target(value.documents);});});return()=>unsub();});
