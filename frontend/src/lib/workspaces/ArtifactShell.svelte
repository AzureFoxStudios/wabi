<script lang="ts">
    ...
    import {workspaceLink,openWorkspace,type Role} from './bridge';
    ...
    let dialog:HTMLDialogElement,error=$state(''),notice=$state(''),busy=$state(false),reviewOpen=$state(false),recoveryOpen=$state(false);
    ...
    let comment=$state(''),proposal=$state(''),suggestion=$state(false),suggestionBase=$state(0),reviewAnchor=$state<string|null>(null);
    let reviewId=crypto.randomUUID(),draftId=$state('review:'+crypto.randomUUID());
    ...
    function showSharing(){const meta=session.record.meta;mode=initialShare||meta?.mode||'live';grants={...(meta?.grants||{})};channelId=meta?.channelId||'';channelRole=meta?.channelRole||'viewer';accessRevision=meta?.accessRevision||0;dialog.showModal();}
    ...
    function stageReview(){
        try{session.stageDraft(draftId,{kind:'review',text:comment,context:{proposal,suggestion,suggestionBase,reviewId,anchor:reviewAnchor},updatedAt:Date.now()});}
        catch(e){error=e instanceof Error?e.message:String(e);}
    }
    function changeComment(value:string){if(!comment&&!proposal)reviewAnchor=selectionAnchor;comment=value;stageReview();}
    function prepareSuggestion(){suggestion=!suggestion;if(suggestion){proposal=session.body.toString();suggestionBase=session.record.meta?.sequence||0;reviewAnchor=selectionAnchor;}else proposal='';stageReview();}
    async function addReview(){
        // Server authorship and permissions apply even when the composer was
        // restored from an old private buffer. A failed request retains it.
        await session.flush();
        await session.review({action:'add',id:reviewId,kind:suggestion?'suggestion':'comment',body:comment,anchor:reviewAnchor,...(suggestion?{proposal,baseSequence:suggestionBase}:{})});
        session.stageDraft(draftId,null);await session.flush();
        if(alive){reviewId=crypto.randomUUID();draftId='review:'+crypto.randomUUID();comment='';proposal='';suggestion=false;reviewAnchor=null;}
    }
    function restoreReview(id:string,draft:PrivateDraft){
        ...
        suggestionBase=Number(draft.context.suggestionBase)||0;reviewId=typeof draft.context.reviewId==='string'?draft.context.reviewId:crypto.randomUUID();reviewAnchor=typeof draft.context.anchor==='string'?draft.context.anchor:null;reviewOpen=true;recoveryOpen=false;
    }
    async function clearComposer(){session.stageDraft(draftId,null);await session.flush();comment='';proposal='';suggestion=false;reviewAnchor=null;reviewId=crypto.randomUUID();draftId='review:'+crypto.randomUUID();}
    function backup(){
        // Include the visible composer even when device storage is full.
        const value=JSON.parse(session.recovery());if(comment||proposal)value.visibleReviewRecovery={comment,proposal,suggestion,suggestionBase,reviewId,anchor:reviewAnchor};
    ...
    async function privateCopy(){
        ...
        if(oncopy)await oncopy(copy);
        else{const id=copy.id,kind=copy.record.kind;await copy.close();openWorkspace(kind==='document'?'documents':kind,{id});}
    }
    ...
    <div class="workspace-body"><main class="workspace-content">{@render children()}</main>
        {#if reviewOpen}<aside class="workspace-reviews" aria-label="Reviews"><h2>Comments and suggestions</h2><button onclick={()=>reviewOpen=false}>Close review</button>
            {#if $sessionState.meta?.role!=='viewer'}
                {#if reviewAnchor}<p>Attached to {describeAnchor(session.doc,reviewAnchor).label}<button onclick={()=>{reviewAnchor=null;stageReview();}}>Use whole document</button></p>{/if}
            ...
            {/if}
            {#each $sessionState.reviews as item(item.id)}<article><strong>{item.authorName}</strong><small> · {item.kind} · {item.state}</small>
...
<dialog bind:this={dialog} class="workspace-dialog"><form onsubmit={event=>{event.preventDefault();void run(share);}}>
    ...
    <label>Person<select bind:value={recipient}><option value="">Choose an account</option>{#each $serverMembers.filter(user=>user.dbUserId) as user(user.dbUserId)}<option value={String(user.dbUserId)}>{user.username}</option>{/each}</select></label>
