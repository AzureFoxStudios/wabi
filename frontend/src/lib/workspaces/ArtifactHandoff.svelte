<script lang="ts">
    ...
    let savedTask = $state<{ id: string; persisted: boolean; flush: () => Promise<boolean>; isCurrent: () => boolean } | null>(null);
    let alive = true;
    ...
    async function discuss() {
        ...
        try {
            ...
            dialog?.close(); switchChannel(target.id); mobileTabQueue.setActiveChannel(target.id);
    ...
    async function readyPlanner(storage: typeof import('$lib/business/deviceStorage')): Promise<void> {
        storage.reloadFromStorage();
        await new Promise<void>((resolve, reject) => {
            let done = false, unsubscribe = () => {};
            const finish = (problem?: Error) => {
                if (done) return; done = true;
                queueMicrotask(() => { clearTimeout(timeout); unsubscribe(); problem ? reject(problem) : resolve(); });
            };
            const timeout = setTimeout(() => finish(new Error('Planner did not finish loading. Open Planner to check its storage, then retry.')), 10000);
            unsubscribe = storage.plannerStorage.subscribe(status => {
                try { check(); } catch (e) { finish(e instanceof Error ? e : new Error(String(e))); return; }
                if (status.loaded) finish();
                else if (status.error) finish(new Error(status.error));
            });
        });
    }
    async function createTask() {
        ...
        try {
            check();
            // A failed write retries the captured original task. A later explicit
            // click after success creates a new task for the current selection.
            if (savedTask && !savedTask.persisted) {
                if (!savedTask.isCurrent()) throw new Error('Planner account changed. The unfinished task remains in its original account.');
                if (!(await savedTask.flush())) throw new Error('Task storage still needs attention. Retry preserves the same task.');
                check(); savedTask.persisted = true;
                notice = 'The previous task is now saved. Use Create private task again to create one for the current selection.';
                return;
            }
            ...
            await readyPlanner(storage); check();
            ...
            savedTask = { id: todo.id, persisted: false, flush: () => captured.session.flush(), isCurrent: captured.isCurrent };
...
{#if notice}<span role="status">{notice} {#if savedTask}<button onclick={() => { if (savedTask?.isCurrent() && session.scope.isCurrent()) mobileTabQueue.openAddonTab('planner'); }}>Open Planner</button>{/if}</span>{/if}
