...
describe('game request ownership',()=>{
  ...
  test('dispose aborts every in-flight request',async()=>{let signal:AbortSignal|undefined;const c=createGameClient(initial,((_,o)=>new Promise((_resolve,reject)=>{signal=o?.signal as AbortSignal;signal.addEventListener('abort',()=>reject(new TypeError('cancelled')));})) as typeof fetch);const pending=c.request('/games/me');c.dispose();expect(signal!.aborted).toBe(true);await expect(pending).rejects.toThrow('Session changed');});
  test('a failed mutation is never automatically retried',async()=>{let count=0;const c=createGameClient(initial,(async()=>{count++;throw new TypeError('network');}) as typeof fetch);await expect(c.request('/games/me','PUT',{})).rejects.toThrow('reload before retrying');expect(count).toBe(1);});
  test('server conflicts preserve a useful error',async()=>{const c=createGameClient(initial,(async()=>new Response('{"error":"Game selections changed. Reload before saving."}',{status:409})) as typeof fetch);await expect(c.request('/games/me','PUT',{})).rejects.toThrow('Reload before saving');});
});
