...
export async function loadBurndownChart(projectId: string): Promise<BurnChartDataPoint[]> {
	if (isRunningInTauri()) {
		...
		>('load_burndown_chart', { chartId: projectId });
...
export async function saveReminders(eventId: string, minutes: Int32Array): Promise<void> {
	if (isRunningInTauri()) {
		// Tauri: Send to Rust backend - merge with existing reminders
		const existing = await invoke<Record<string, number[]>>('load_reminders');
		existing[eventId] = Array.from(minutes);
...
export async function loadReminders(eventId: string): Promise<Int32Array | null> {
	if (isRunningInTauri()) {
		const data = await invoke<Record<string, number[]>>('load_reminders');
