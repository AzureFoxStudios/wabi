...
const orphanIceCandidates = new Map<string, Array<{ candidate: RTCIceCandidateInit; at: number; accepts: (state: PeerConnectionState) => boolean }>>();
...
export function queueIceCandidate(
	...
	accepts: (state: PeerConnectionState) => boolean = () => true
): void {
	...
	if (!state) {
		...
		if (parked.length < ORPHAN_ICE_CAP) parked.push({ candidate, at: Date.now(), accepts });
	...
	if (state.hasRemoteDescription) {
		...
		if (state.iceCandidateQueue.length < ORPHAN_ICE_CAP) state.iceCandidateQueue.push(candidate);
...
export function flushOrphanIceCandidates(
	...
	if (!state) return;
	const now = Date.now();
	const fresh = parked.filter((p) => now - p.at < ORPHAN_ICE_MAX_AGE_MS && p.accepts(state));
