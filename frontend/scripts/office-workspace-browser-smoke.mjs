...
// Production editors and persistence, separate browser profiles, real Authority API.
// Only identity/bootstrap and navigation are fixture adapters; no fake shared transport.
...
const f=name=>path.join(fixture,name);
const mod=relative=>JSON.stringify(path.join(frontend,relative));
let server,browser;
let checks=0;
const failures=[];
try{
    ...
    await writeFile(f('App.svelte'),`<script>
        import WorkspaceHost from ${mod('src/lib/workspaces/WorkspaceHost.svelte')};
        import {active} from './tabs.js';
        import {workspaceToolFromTab} from ${mod('src/lib/workspaces/bridge.ts')};
        $: tool=workspaceToolFromTab($active);
        </script>{#if tool}<WorkspaceHost {tool}/>{/if}`);
    await writeFile(f('main.js'),`import {mount} from 'svelte';import App from './App.svelte';
        import {openWorkspace,captureScope} from ${mod('src/lib/workspaces/bridge.ts')};
        import {account} from './identity.js';mount(App,{target:document.getElementById('app')});
        window.workspaceTest={open:openWorkspace,account,
            async records(kind){const m=await import(${mod('src/lib/workspaces/session.ts')});return m.listLocal(await captureScope(),kind);},
            async record(id){const m=await import(${mod('src/lib/workspaces/session.ts')});const r=await m.readLocal(await captureScope(),id);return r?{id:r.id,pending:Object.keys(r.pending).length,meta:r.meta}:null;},
            ...
            async api(route,body){const r=await fetch('/api/workspace'+route,{method:body===undefined?'GET':'POST',headers:{Authorization:'Bearer '+account.token,'Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)});return{status:r.status,body:await r.json()};}};`);
    ...
    await writeFile(f('index.html'),'<!doctype html><html><head><meta charset="utf-8"><style>html,body,#app{height:100%;width:100%;margin:0}body{font-family:system-ui;background:#161826;color:#eef0f8}</style></head><body><div id="app"></div><script type="module" src="/main.js"></script></body></html>');
    const alias=['notes/scope','serverUrl','authSession','presenceIdentity','channelStore'].map(name=>({find:'$lib/'+name,replacement:f('identity.js')}));
    alias.push({find:'$lib/mobileTabQueue',replacement:f('tabs.js')},{find:'@wabi/workspace-sheets',replacement:path.join(frontend,'src/lib/workspaces/sheets/addon.ts')},{find:'@wabi/workspace-present',replacement:path.join(frontend,'src/lib/workspaces/present/addon.ts')},{find:'$lib',replacement:path.join(frontend,'src/lib')});
    server=await createServer({configFile:false,root:fixture,plugins:[svelte({configFile:false})],resolve:{dedupe:['svelte','yjs'],alias},worker:{format:'es'},define:{__WABI_WORKSPACE_PACKAGED__:JSON.stringify({sheets:true,present:true})},optimizeDeps:{rolldownOptions:{tsconfig:false}},server:{host:'127.0.0.1',port:0,fs:{allow:[frontend]},proxy:{'/api':host.authorityUrl}},logLevel:'warn'});
    ...
    for(const engineName of (process.env.WORKSPACE_BROWSER||'chromium').split(',')){
        browser=await(engineName==='firefox'?firefox:chromium).launch({headless:true,...(engineName==='chromium'?{args:['--no-sandbox']}:{})});
        ...
        const a=await ca.newPage(),b=await cb.newPage(),c=await cc.newPage();const errors=[];
        for(const page of [a,b,c]){page.on('pageerror',e=>errors.push(e.message));page.setDefaultTimeout(20000);}
        try{
            await a.goto(origin+'?account=0&tool=documents');
            await a.getByRole('button',{name:'New document',exact:true}).click();
            ...
            const records=await a.evaluate(()=>window.workspaceTest.records('document'));const id=records[0].id;
            ...
            await a.getByLabel('Person',{exact:true}).selectOption(String(host.accounts[1].id));await a.getByRole('button',{name:'Add person',exact:true}).click();
            ...
            await ca.setOffline(true);await a.locator('.cm-content').click();await a.keyboard.press('Control+End');await a.keyboard.insertText(' — Alice offline');
            await a.waitForFunction(async id=>(await window.workspaceTest.record(id))?.pending>0,id);checks++;
            await b.locator('.cm-content').click();await b.keyboard.press('Control+End');await b.keyboard.insertText(' — Bob online');
            await b.waitForFunction(async id=>(await window.workspaceTest.record(id))?.pending===0,id);
            ...
            const starting=a.waitForResponse(r=>r.url().endsWith('/api/workspace/presentations')&&r.request().method()==='POST');await a.getByRole('button',{name:'Present to channel…',exact:true}).click();await a.getByRole('button',{name:'Start approved presentation',exact:true}).click();const response=await starting;assert.equal(response.status(),200);const session=await response.json();assert.equal(session.slides.length,2);checks++;
            await b.goto(origin+'?account=1&tool=audience');await b.waitForFunction(()=>!!window.workspaceTest);const payloads=[];b.on('response',r=>{if(r.url().includes('/api/workspace/'))void r.text().then(t=>payloads.push(t)).catch(()=>{});});await b.evaluate(id=>window.workspaceTest.open('audience',{sessionId:id}),session.id);await b.getByRole('heading',{name:'Visible first slide',exact:true}).waitFor();checks++;
        ...
        }catch(error){failures.push(`${engineName}: ${error.stack||error}`);for(const[name,page]of [['alice',a],['bob',b],['carol',c]]){await page.screenshot({path:path.join(artifacts,`${engineName}-failure-${name}.png`)}).catch(()=>{});console.error(`${engineName}/${name}: `+(await page.locator('body').innerText().catch(()=>'' )).slice(0,6000));}console.error('Page errors: '+JSON.stringify(errors));}
