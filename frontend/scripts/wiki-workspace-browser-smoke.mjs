...
try {
    ...
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.openRightPanel('wiki'); });
    await panelWiki.locator('.wiki-edit-body').waitFor();
    assert.equal(await panelWiki.locator('.wiki-edit-body').inputValue(), 'Independent panel draft', 'panel draft restores after closing');
    assert.equal(await centerWiki.locator('.wiki-edit-body').inputValue(), 'Unsaved draft must survive refresh');
    let releasePanelSave, panelSaveStarted;
    const panelSaveGate = new Promise(resolve => { releasePanelSave = resolve; });
    const panelSaveReady = new Promise(resolve => { panelSaveStarted = resolve; });
    let panelSaveRequests = 0;
    await page.route('**/api/wiki/*/pages/*', async route => {
        if (route.request().method() !== 'PUT') return route.continue();
        panelSaveRequests += 1; panelSaveStarted(); await panelSaveGate; await route.continue();
    });
    await panelWiki.getByRole('button', { name: 'Save', exact: true }).click();
    await panelSaveReady;
    await panelWiki.locator('.wiki-edit-body').fill('Typing after the submitted snapshot');
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.closeRightPanel(); });
    await panelWiki.waitFor({ state: 'hidden' });
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.openRightPanel('wiki'); });
    await panelWiki.locator('.wiki-edit-body').waitFor();
    assert.equal(await panelWiki.getByRole('button', { name: 'Save', exact: true }).isDisabled(), true);
    releasePanelSave();
    await page.waitForFunction(() => !document.querySelector('.right-panel-embedded .wiki-edit-save-btn')?.disabled);
    assert.equal(await panelWiki.locator('.wiki-edit-body').inputValue(), 'Typing after the submitted snapshot', 'late acknowledgement preserves later typing');
    assert.equal(panelSaveRequests, 1);
    await page.unroute('**/api/wiki/*/pages/*');
    page.once('dialog', dialog => dialog.accept());
    await panelWiki.getByRole('button', { name: 'Cancel', exact: true }).click();
    await panelWiki.getByRole('button', { name: '+ New Page', exact: true }).click();
    await panelWiki.locator('.wiki-edit-title').fill('Recovered new page');
    await panelWiki.locator('.wiki-edit-body').fill('New page draft in the panel');
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.closeRightPanel(); });
    await panelWiki.waitFor({ state: 'hidden' });
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.openRightPanel('wiki'); });
    await panelWiki.locator('.wiki-edit-body').waitFor();
    assert.equal(await panelWiki.locator('.wiki-edit-title').inputValue(), 'Recovered new page');
    assert.equal(await panelWiki.locator('.wiki-edit-body').inputValue(), 'New page draft in the panel');
    let releasePanelCreate, panelCreateStarted;
    const panelCreateGate = new Promise(resolve => { releasePanelCreate = resolve; });
    const panelCreateReady = new Promise(resolve => { panelCreateStarted = resolve; });
    let panelCreates = 0;
    await page.route('**/api/wiki/*/pages', async route => {
        if (route.request().method() !== 'POST') return route.continue();
        panelCreates += 1; panelCreateStarted(); await panelCreateGate; await route.continue();
    });
    await panelWiki.getByRole('button', { name: 'Create', exact: true }).click();
    await panelCreateReady;
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.closeRightPanel(); });
    await panelWiki.waitFor({ state: 'hidden' });
    await page.evaluate(async () => { const { layoutStore } = await import('/src/lib/layoutStore.ts'); layoutStore.openRightPanel('wiki'); });
    await panelWiki.getByRole('button', { name: 'Creating…', exact: true }).waitFor();
    assert.equal(await panelWiki.getByRole('button', { name: 'Creating…', exact: true }).isDisabled(), true);
    releasePanelCreate();
    await panelWiki.locator('.wiki-content-header-title').filter({ hasText: 'Recovered new page' }).waitFor();
    await panelWiki.locator('.wiki-content-body').filter({ hasText: 'New page draft in the panel' }).waitFor();
    ...
    await page.evaluate(async ({ first, second }) => {
        ...
        try {
            ...
            if (read(b.wikiPagesStore).length !== 2) throw new Error('independent view lost its list');
            ...
            a.dispose();
            if (read(b.wikiPagesStore).length !== 2) throw new Error('disposing one view cleared another');
            b.dispose();
    ...
    await page.evaluate(async id => {
        ...
        if (read(view.wikiPagesStore).length !== 2) throw new Error('logout fixture not loaded');
        const original = window.fetch;
        let release, started;
        const gate = new Promise(resolve => { release = resolve; });
        const ready = new Promise(resolve => { started = resolve; });
