...
| E2EE for P2P transfers | Encrypted DataChannel for DMs | Missing | No encryption is applied to the DataChannel or the current HTTP DM upload path. The frontend contains dormant encrypted-attachment scaffolding, but production uploads are server-readable. | Future: implement and verify key negotiation plus chunk/blob encryption before making any E2EE claim. | Missing — future work. |
