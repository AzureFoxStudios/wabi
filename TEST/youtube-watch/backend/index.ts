...
function createDefaultRoom(channelId: string): ChannelWatchState {
  return {
    channelId,
    positionSec: 0,
    isPlaying: false,
...
function extractChannelId(data: WatchPayload): string | null {
...
function canControl(room: ChannelWatchState, socket: Socket): boolean {
...
function applyAction(ctx: PluginContext, room: ChannelWatchState, action: WatchAction): void {
  switch (action.type) {
    case 'play': {
      room.isPlaying = true;
      if (typeof action.payload?.positionSec === 'number') {
        room.positionSec = Math.max(0, action.payload.positionSec);
      }
      break;
    }
    case 'pause': {
  ...
  emitRoomState(ctx, room.channelId);
  void persistRooms();
}
...
function createAction(
  ...
  return {
    ...
    requestedBy,
    requestedAt: now,
    scheduledAt: now + DEFAULT_COMMIT_WINDOW_MS,
    payload
  };
}
...
function resolveVote(ctx: PluginContext, room: ChannelWatchState): void {
  ...
  if (passed) {
    applyAction(ctx, room, vote.action);
  } else {
    emitRoomState(ctx, room.channelId);
    void persistRooms();
  }
  ...
  onConnection(socket: Socket, ctx: PluginContext) {
    socket.emit('watch:capabilities', {
      ...
      moderation: {
        ...
        maxQueueItemsPerUser: MAX_QUEUE_ITEMS_PER_USER
      },
      sync: {
        commitWindowMs: DEFAULT_COMMIT_WINDOW_MS,
        notes: 'Phase 1 scaffold: adaptive jitter windows and drift engine planned next.'
      }
    });
  },
  ...
  onDisconnect(socket: Socket) {
    for (const key of submitCooldown.keys()) {
      if (key.endsWith(`:${socket.id}`)) {
        submitCooldown.delete(key);
      }
    }
  },

  socketHandlers: {
    'watch:get-state': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) {
        socket.emit('watch:error', { message: 'channelId is required' });
        return;
      }
      socket.emit('watch:state', getRoom(channelId));
    },

    'watch:add': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const raw = typeof data?.url === 'string' ? data.url : (typeof data?.videoId === 'string' ? data.videoId : '');
      if (!channelId || !raw) {
        socket.emit('watch:error', { message: 'channelId and url/videoId are required' });
        return;
      }

      const videoId = extractYouTubeVideoId(raw);
      if (!videoId) {
        socket.emit('watch:error', { message: 'Invalid YouTube URL or video ID' });
        return;
      }
      ...
      if (cooldownUntil > now) {
        socket.emit('watch:error', { message: 'Submitting too quickly. Please wait a moment before adding another link.' });
        return;
      }

      const requestCount = bumpVideoInterest(room, videoId, username);
      if (isDuplicateVideo(room, videoId)) {
        socket.emit('watch:error', { message: `That video is already current, queued, or pending (requested ${requestCount} times).` });
      ...
      if (ownQueueCount >= MAX_QUEUE_ITEMS_PER_USER) {
        socket.emit('watch:error', { message: `Queue limit reached (${MAX_QUEUE_ITEMS_PER_USER} per user).` });
        return;
      }
      ...
      if (room.queueModerated && !requesterCanBypassModeration) {
        ...
        room.updatedAt = Date.now();
        room.updatedBy = item.requestedBy;
        emitRoomState(ctx, channelId);
        await persistRooms();
      ...
      if (!room.currentVideoId) {
        room.currentVideoId = item.videoId;
        room.queue = room.queue.filter((q) => q.id !== item.id);
      }
      room.updatedAt = Date.now();
      room.updatedBy = item.requestedBy;
      emitRoomState(ctx, channelId);
    ...
    'watch:queue:moderation:set': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      ...
      if (blocked) {
        socket.emit('watch:error', { message: blocked });
        return;
      }

      room.queueModerated = enabled;
      room.updatedAt = Date.now();
      room.updatedBy = getUsername(ctx, socket);
      emitRoomState(ctx, channelId);
      await persistRooms();
    },

    'watch:queue:approve': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      ...
      if (!room.currentVideoId) {
        room.currentVideoId = approved.videoId;
        room.queue = room.queue.filter((q) => q.id !== approved.id);
      }
    ...
    'watch:queue:reject': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const queueId = typeof data?.queueId === 'string' ? data.queueId.trim() : '';
      if (!channelId || !queueId) return;
    ...
    'watch:queue:remove': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const queueId = typeof data?.queueId === 'string' ? data.queueId.trim() : '';
      if (!channelId || !queueId) return;
      const room = getRoom(channelId);
    ...
    'watch:queue:clear': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);
      const blocked = queueControlBlocked(room, socket);
    ...
    'watch:queue:move': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const queueId = typeof data?.queueId === 'string' ? data.queueId.trim() : '';
      ...
      const moved = moveQueueItem(room.queue, queueId, direction as QueueMoveDirection);
      room.queue = moved;
      room.updatedAt = Date.now();
      room.updatedBy = getUsername(ctx, socket);
      emitRoomState(ctx, channelId);
    ...
    'watch:queue:play-next': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const queueId = typeof data?.queueId === 'string' ? data.queueId.trim() : '';
      if (!channelId || !queueId) return;
    ...
    'watch:queue:play-now': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const queueId = typeof data?.queueId === 'string' ? data.queueId.trim() : '';
      if (!channelId || !queueId) return;
      ...
      if (room.currentVideoId) {
        room.queue.unshift({
          ...
          title: undefined,
          requestedBy: getUsername(ctx, socket),
          requestedAt: Date.now()
        });
      }

      room.currentVideoId = target.videoId;
      room.positionSec = 0;
      room.isPlaying = false;
      room.updatedAt = Date.now();
      room.updatedBy = getUsername(ctx, socket);
    ...
    'watch:play': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);
      ...
      if (room.controlMode === 'vote') {
        socket.emit('watch:error', { message: 'Room is in vote mode. Start a vote instead.' });
        return;
      }
      if (!canControl(room, socket)) {
        socket.emit('watch:error', { message: 'Control is presenter-locked' });
        return;
      }

      applyAction(ctx, room, createAction('play', channelId, requestedBy, {
        positionSec: Number(data?.positionSec ?? room.positionSec)
      }));
    },

    'watch:pause': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);
      const requestedBy = getUsername(ctx, socket);
      ...
      applyAction(ctx, room, createAction('pause', channelId, requestedBy, {
        positionSec: Number(data?.positionSec ?? room.positionSec)
      }));
    },

    'watch:seek': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);
      const requestedBy = getUsername(ctx, socket);
      ...
      if (!Number.isFinite(positionSec)) {
        socket.emit('watch:error', { message: 'positionSec must be a number' });
        return;
      }
    ...
    'watch:skip': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);
      const requestedBy = getUsername(ctx, socket);
    ...
    'watch:mode:set': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const nextMode = typeof data?.mode === 'string' ? data.mode : '';
      if (!channelId || !['open', 'presenter', 'vote'].includes(nextMode)) {
        socket.emit('watch:error', { message: 'channelId and valid mode are required' });
        return;
      }

      const room = getRoom(channelId);
      if (room.presenterUserId && room.presenterUserId !== socket.id) {
        socket.emit('watch:error', { message: 'Only presenter can change mode' });
        return;
      }

      room.controlMode = nextMode as WatchControlMode;
      room.updatedAt = Date.now();
      room.updatedBy = getUsername(ctx, socket);
      emitRoomState(ctx, channelId);
      await persistRooms();
    ...
    'watch:presenter:set': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      if (!channelId) return;
      const room = getRoom(channelId);

      if (room.presenterUserId && room.presenterUserId !== socket.id) {
        socket.emit('watch:error', { message: 'Presenter lock is already owned' });
        return;
      }

      room.presenterUserId = socket.id;
      room.controlMode = 'presenter';
      room.updatedAt = Date.now();
      room.updatedBy = getUsername(ctx, socket);
      emitRoomState(ctx, channelId);
      await persistRooms();
    ...
    'watch:vote:start': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      const type = typeof data?.actionType === 'string' ? data.actionType : '';
      if (!channelId || !['play', 'pause', 'seek', 'skip'].includes(type)) {
        socket.emit('watch:error', { message: 'channelId and valid actionType are required' });
        return;
      }

      const room = getRoom(channelId);
      if (room.pendingVote) {
        socket.emit('watch:error', { message: 'A vote is already active in this room' });
        return;
      }

      const requestedBy = getUsername(ctx, socket);
      const action = createAction(type as WatchActionType, channelId, requestedBy, {
        positionSec: Number(data?.positionSec ?? room.positionSec)
      });
      ...
      setTimeout(() => {
        ...
        if (pending && pending.id === room.pendingVote?.id && Date.now() >= pending.expiresAt) {
          resolveVote(ctx, current);
        }
      }, VOTE_DURATION_MS + 50);
    },

    'watch:vote:cast': async (socket: Socket, data: WatchPayload, ctx: PluginContext) => {
      const channelId = extractChannelId(data);
      ...
      const vote = room.pendingVote;
      if (!vote) {
        socket.emit('watch:error', { message: 'No active vote in this room' });
        return;
      }

      vote.yes = vote.yes.filter((id) => id !== socket.id);
      vote.no = vote.no.filter((id) => id !== socket.id);
      if (approve) vote.yes.push(socket.id);
      else vote.no.push(socket.id);

      room.updatedAt = Date.now();
      emitRoomState(ctx, channelId);
      await persistRooms();

      if (vote.yes.length >= vote.threshold) {
        resolveVote(ctx, room);
        return;
      }

      const channelUsers = activeChannelUserCount(ctx, channelId);
      if (vote.no.length > channelUsers - vote.threshold) {
        resolveVote(ctx, room);
      }
    },

    'watch:sync:ping': (socket: Socket, data: WatchPayload) => {
      socket.emit('watch:sync:pong', {
  ...
  routes: [
    {
      ...
      handler: async (_req, res) => {
        res.json({
          ...
          notes: 'Phase 1 scaffold. Adaptive jitter windows/drift correction are next-phase work.'
        });
      }
    }
  ]
