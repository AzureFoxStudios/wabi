...
- "Can't log in, yet console says I'm logged in" (socket `Join as:` + Init received + theme prefs load from server, but the Login form never hands off) → a module-eval crash rejected the **app-bundle dynamic import** (`ensureLayoutRouter` in `routes/+page.svelte`); `loggedIn=true` but `LayoutRouterCmp` stays null so `<Login>` re-renders forever. Look for `Uncaught (in promise) <Error>` right after theme init. Real case 2026-09-02: `Prism is not defined` — prismjs grammar components inlined eagerly (bare global `Prism` refs) into a shared chunk while the core sat in a lazy `__commonJS` wrapper nobody called first; new prism import sites (whiteboard codeHighlight / LoreDiffViewer) reshuffled the chunk graph into the broken order. Fix: `frontend/src/lib/prism.ts` is the ONLY module allowed to touch prismjs — it pins `globalThis.Prism` and loads grammars via sequential dynamic `import('prismjs/components/...')`; `ensureLayoutRouter()` chains `ensurePrismGrammars()` before importing LayoutRouter (with a visible catch + retry); detached route awaits it in onMount. Regression gate: `bun test src/lib/prism.test.ts` (grammar registration + source-scan that no file outside `lib/prism.ts` imports prismjs).
