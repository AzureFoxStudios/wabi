...
> **Status:** 799 tests passing (656 wabidb + 99 cargo workspace + 44 wabi-server). Replay engine + persistence policy implemented. **Latest session (2026-06-28):** Added `ReplayEnvelope` write-path, `replay_projections()` with snapshot watermark skip, JSON snapshot save/load/checkpoint (every 1000 events via dispatcher), 4 noop handler stubs (`reaction_removed`, `member_joined`, `member_left`, `channel_renamed`). **Updated status tracking (2026-06-28):** Re-audited all 99 cards against actual source tree to produce realistic completion counts.
