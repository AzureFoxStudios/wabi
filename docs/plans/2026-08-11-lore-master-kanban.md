...
- 2026-08-17: **Code-workflow pass landed** (full record: `docs/plans/2026-08-17-lore-code-sync.md`). Backend: revision-pinned downloads via a revcache, ETag/If-Match optimistic concurrency (409 `StaleEtag`), real `merge_branch`, honest WDB event writes, ignore-check-before-copy, parser hardening, blob-size + auto-create config enforced, mirror/editor-bridge/script-runner made real-or-honest, health check TCPs the lore server. Sync server: `/manifest` + cursor change feed (`lore_file_change` projection), server-minted hashed connect tokens (`lore_tokens` projection, scope guard), live `lore:file-changed` socket events. New `crates/wabi-sync` CLI (login/link/watch/push/pull/lock, etag-baseline three-way sync, `.wabi-conflict-*` copies). Frontend: CodeMirror 6 viewer+editor with conflict resolution, Prism-highlighted diffs with line numbers, live socket refresh, Connect panel mints real tokens + prints wabi-sync commands, FilesWorkspace on LoreFileTree with concurrent batched uploads. Tests: wabi-lore 28, wabidb lore 13, api::lore 9 (stub-CLI e2e), wabi-sync 8; `bun run check` 0 errors. The lore skill file referenced above does not exist on this machine — projection changes documented in the 08-17 plan doc instead.
- 2026-08-27: **Setup-UX pass landed, TUI-first** (full record: `docs/plans/2026-08-27-lore-setup-ux-tui.md`). Channel-as-repo made real end-to-end: auto-created repos named after their channel (slug) instead of `ch-{id}`; `import_from_git` adopts empty auto-created repos instead of 409 (pristine-tree walker); `PUT …?stageOnly=true` stages without committing so a folder push seals into ONE snapshot revision. New TUI `Screen::Lore` (repo browser + `v` text preview) and `:lore new/push/import/health` commands — the device-link flow is one command, no token copy-paste. Web honesty fixes: dead lore-server URL field removed from LoreConnectModal, mode-aware health advice, `/lore` chat commands parse `ch_{hex}` correctly and stopped merge-lying as "switch" and empty-PUT-destroying as "stage". Also fixed in-pass: native `list_files` now walks the working tree (status labels overlay) — committed repos previously browsed as 3 furniture files; TUI no longer renders a fake `● user` header for a remembered username without a token; TUI repo-class normalization handles the `{"type":"native"}` tagged-object wire shape. Tests: wabi-lore 34, api::lore 12. Wabi-on-wabi fixture LIVE on Tim: lore channel `wabi` (1120) hosts the full worktree — 2,534 files, one "Initial import" snapshot, head `c6c9de51…`, verified across a restart.

---

# Current working state

- **Deployed to Tim 2026-08-27:** release binary SHA `ab3aaf92ce104561…` (second swap; first was `140e4709…`); container recreated and healthy; host `:3001`, Caddy `:8088`, and public `https://wabi.chat/health` all 200; lore addon health `{"status":"ok"}` under embedded mode, lore CLI 0.8.6+373. Persistent `/var/wabi/lore` retained channels 2/3/5/61/97/1120. Note: `docker logs` is empty on this deployment (logs not captured); rehydration verified functionally via the lore API (WDB-registered repos respond after restart). Base = `origin/main` `1105c37` + cherry-picked bazzite `367af1d4` (`wabidb-pong &data` — origin/main did not compile without it).
