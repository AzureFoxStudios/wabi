...
2. **Content gets priority.** The active conversation, document or canvas owns the main area. Empty People panels, repeated sidebars and decorative effects cannot dominate the first view. Secondary panels are intentional, discoverable and easy to dismiss.
3. **One action has one understandable path.** Consolidate duplicate navigation and competing editors through the existing workspace/dock system. Notes and its scratchpad share data before their controls are consolidated.
...
| U02 — Shell and navigation contract | One predictable open/dock/return behavior, consistent workspace headers, intentional overflow, saved layout and panel preview ownership. Reduce competing default panels and repeated navigation. Diagnose/fix the observed People takeover when Notes is selected on mobile. Preserve recent mobile picker changes. | B02 | The active content gets adequate room; every destination stays reachable at narrow widths; closing/previewing/resizing does not unexpectedly reopen a panel, lose the selected workspace or discard drafts. |
...
| MEDIUM | Notes / desktop and phone composition | Competing sidebars/panels squeeze writing; the populated editor uses a shallow area above a large unused background. Color/date controls outrank the note's identity, and mobile labels the selected Notes surface “Chat.” | Prioritize title and writing area, give the editor the available height, simplify its toolbar, and make navigation labels match their role. Unify scratchpad data before consolidating its controls. | Content needs space and one understandable editing model. The opening mobile repair restores reachability; it does not finish the Notes redesign. |
