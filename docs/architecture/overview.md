...
- **Sequencer/transactions**: one writer; every commit is a `CommandCommit` with `EventToWrite`s; fsync + crash recovery; see `docs/architecture/` + wabidb skills for depth.
