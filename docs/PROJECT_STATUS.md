...
The candidate also scopes device-local Planner data to its server/account, retains competing or failed-save drafts, and replaces destructive imports with validated additive imports. Planner server sync is unavailable; recurring occurrence expansion remains unfinished. See [Local Planner](features/LOCAL_PLANNER.md).
