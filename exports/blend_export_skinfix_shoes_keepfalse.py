...
def patch_gltf_temp_image_encoder():
    ...
    def safe_encode_temp_image(tmp_image, file_format, export_settings):
        tmpdirname = tempfile.mkdtemp(prefix='gltfimg_', dir=temp_root)
        ext = ext_by_format.get(file_format, '.img')
        tmpfilename = os.path.join(tmpdirname, 'img' + ext)
...
face_skin = bpy.data.images.get('Head uv unwraped.png')
